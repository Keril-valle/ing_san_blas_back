...
describe('BusquedaNormalizadaController', () => {
  it('delegates the search filters to the normalized service', async () => {
    ...
    const result = { items: [], total: 0, page: 1, pageSize: 20, totalPages: 1 };
    const buscar = jest.fn().mockResolvedValue(result);
    const controller = new BusquedaNormalizadaController({ buscar } as unknown as BusquedaNormalizadaService);
  ...
  it('delegates create, update, detail and delete operations', async () => {
    ...
    await expect(controller.crear(createDto as never)).resolves.toEqual({ id: 1 });
